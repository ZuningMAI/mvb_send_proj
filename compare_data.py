...
def parse_log_frame(line):
    """
    解析日志中的帧数据行
    格式: [2025-11-11 18:30:24.814] [FRAME] PD=0XA0 | 数据: 7e 21 04 00 a0 00 00 26 50 00 06 00 13 00 0d 00 0c 10 69 00 00 08 70 00 5f 0c 9e 04 09 00 00 00 00 00 00 00 00 55 97 7e | 发送: 40/40 字节
    """
    # 使用简单的字符串处理而不是复杂的正则表达式
    try:
        # 提取时间戳
        ...
        if pd_end == -1:  # 如果没有找到空格，查找 |
            pd_end = line.find('|', pd_start)
        if pd_end == -1:  # 如果没有找到 |，则到行尾
            pd_end = len(line)
        pd_type = line[pd_start:pd_end].strip()
        
        # 提取数据
        data_start = line.find('数据: ') + 3  # "数据:" 后面的内容
        ...
        data_hex = data_part.replace(' ', '').upper()
...
def parse_txt_frame(line):
    """
    解析txt文件中的帧数据行
    格式: [18:30:24.819] PD:0XA0 原始数据:7E 21 04 00 A0 00 00 26 50 00 06 00 13 00 0D 00 0C 10 69 00 00 08 70 00 5F 0C 9E 04 09 00 00 00 00 00 00 00 00 55 97 7E
    """
    try:
        # 提取时间
        ...
        # 提取数据
        data_start = line.find('原始数据:') + 5  # "原始数据:" 后面的内容
...
def compare_frames(log_frames, txt_frames, time_tolerance=1.0):
    ...
    for log_frame in log_frames:
        ...
        if pd_type in txt_time_index:
            for txt_frame in txt_time_index[pd_type]:
...
def main():
    parser = argparse.ArgumentParser(description='比较.log文件和data.txt中的MVB帧数据')
    parser.add_argument('--log', default='bin/Log/Y0.log', help='日志文件路径')
    parser.add_argument('--data-dir', default='dataY0/originData/2025-11-11', help='data.txt文件目录')
    ...
    for pd_type in sorted(all_pd_types):
        ...
        if unmatched_log:
            print(f"\n{pd_type} 类型在日志中未匹配的帧:")
            for frame in unmatched_log[::]:  # 只显示前5个
        ...
        if unmatched_txt:
            ...
            for frame in unmatched_txt[::]:  # 只显示前5个
